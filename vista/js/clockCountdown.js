...
clockCountdown.intervalInfo =  {
	'segundos':{'limit':60,'next': 'minutos'},
	'minutos':{'limit':60,'next': 'horas'},
	'horas':{'limit':24,'next': 'dias'},
	'dias':{'limit':365,'next': null}
}
/*
...
clockCountdown.update = function (id_clock){
		...
		change =  {'segundos': true,'minutos': false,'horas':false,'dias':false};
		clock.remaining.segundos-=1;
		for(label in clockCountdown.intervalInfo){
			...
			if( clock.remaining[label] == clockCountdown.intervalInfo[label].limit || clock.remaining[label]%10==0){
				...
				if( label == 'horas' && clock.remaining[label] == clockCountdown.intervalInfo[label].limit ){
					n++;
				}//endIf
